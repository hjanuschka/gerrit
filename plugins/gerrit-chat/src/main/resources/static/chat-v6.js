  ...
  class GerritChatWidget extends HTMLElement {
    ...
    render() {
      this.shadowRoot.innerHTML = `
        ...
        <button class="fab" type="button" title="Open AI chat">CHAT</button>
    ...
    async reload() {
      ...
      try {
        const state = await api.get(`/changes/${this.changeNum}/gerrit-chat~chat`);
    ...
    async sendMessage() {
      ...
      try {
        const state = await api.post(`/changes/${this.changeNum}/gerrit-chat~chat`, {message: text});
        this.permissions = state.permissions || {};
        this.messages = state.messages || [];
        this.renderMessages();
        this.updateComposer();

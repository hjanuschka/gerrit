Gerrit.install(plugin => {
    ...
    render() {
      this.shadowRoot.innerHTML = `
        <style>
          ...
          .body.markdown gr-formatted-text {
            --gr-formatted-text-prose-max-width: 100%;
            display: block;
            font-size: 12px;
            line-height: 1.4;
          }
  ...
  function renderMarkdownFragment(text) {
    if (customElements.get('gr-formatted-text')) {
      const el = document.createElement('gr-formatted-text');
      el.markdown = true;
      el.content = text || '';
      return el;
    }

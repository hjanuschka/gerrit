Gerrit.install(plugin => {
  ...
  class GerritChatWidget extends HTMLElement {
    ...
    render() {
      this.shadowRoot.innerHTML = `
        <style>
          ...
          .account-hovercard.open {
            display: block;
          }

          .account-hovercard-head {
            display: flex;
            align-items: center;
            gap: 8px;
            margin-bottom: 8px;
        ...
        <div class="account-hovercard" aria-hidden="true"></div>
      `;
      ...
      this.accountHovercardEl = this.shadowRoot.querySelector('.account-hovercard');
      this.accountHovercardHideTimer = null;
      this.accountCache = this.accountCache || new Map();

      this.accountHovercardEl.addEventListener('mouseenter', () => {
        this.cancelHideAccountHovercard();
      });
      this.accountHovercardEl.addEventListener('mouseleave', () => {
        this.scheduleHideAccountHovercard();
      });
    ...
    startProgress() {
      ...
      this.progressTimer = window.setInterval(() => {
        ...
        this.updatePendingMessageDisplay();
      }, 1000);
    }
    ...
    startAssistantStream(messageId, fullText) {
      ...
      const tick = () => {
        ...
        this.updateStreamedAssistantBody();
      };
    ...
    updatePendingMessageDisplay() {
      const pending = this.listEl.querySelector('.msg.pending');
      if (!pending) return;

      const stepEl = pending.querySelector('.pending-step-text');
      const elapsedEl = pending.querySelector('.pending-elapsed-text');
      const toolTrailEl = pending.querySelector('.pending-tool-trail');

      const step = this.progressSteps[this.progressStepIndex] || 'working';
      if (stepEl) {
        stepEl.textContent = `Working… ${formatProgressStepForStatus(step)}`;
      }
      if (elapsedEl) {
        elapsedEl.textContent = `Elapsed ${formatElapsed(Date.now() - this.progressStartedAt)}`;
      }
      if (toolTrailEl) {
        if (this.progressToolTrail.length) {
          toolTrailEl.textContent = this.progressToolTrail.map(name => `• ${name}`).join('\n');
          toolTrailEl.style.whiteSpace = 'pre-wrap';
        } else {
          toolTrailEl.textContent = 'Waiting for first tool call…';
          toolTrailEl.style.whiteSpace = '';
        }
      }
    }

    updateStreamedAssistantBody() {
      if (!this.assistantStream || !this.assistantStream.id) return;

      const target = this.listEl.querySelector(
        `[data-msg-id="${cssEscape(this.assistantStream.id)}"] .body.markdown`,
      );
      if (!target) return;

      target.textContent = '';
      target.appendChild(renderMarkdownFragment(this.assistantStream.currentText || '', this.markedReady));
    ...
    renderAuthorBadge(msg, roleName) {
      ...
      if (roleName === 'user' && accountId) {
        ...
        avatar.alt = authorName ? `${authorName} avatar` : 'avatar';
        avatar.src = this.resolveAvatarForMessage(msg, accountId, authorName);
        avatar.referrerPolicy = 'no-referrer';

        const link = document.createElement('a');
        link.className = 'account-link';
        link.textContent = authorName || `a/${accountId}`;
        link.href = `/q/owner:${encodeURIComponent(authorName || String(accountId))}`;
        link.target = '_blank';
        link.rel = 'noopener noreferrer';

        link.addEventListener('mouseenter', e => {
          this.showAccountHovercard(e.currentTarget, msg);
        });
        ...
        this.prefetchAccountForMessage(msg, accountId, authorName);
        return wrap;
      }
    ...
    resolveAvatarForMessage(msg, accountId, authorName) {
      const cached = this.accountCache.get(accountId);
      if (cached && cached.avatarUrl) return cached.avatarUrl;
      return getAuthorAvatarUrl(msg, accountId, authorName);
    }

    prefetchAccountForMessage(msg, accountId, authorName) {
      if (!accountId || this.accountCache.has(accountId)) return;

      const base = {
        id: accountId,
        name: authorName || `a/${accountId}`,
        email: getAuthorEmail(msg, accountId, authorName),
        username: null,
      };
      base.avatarUrl = getAuthorAvatarUrl(msg, accountId, authorName);
      this.accountCache.set(accountId, base);

      api
        .get(`/a/accounts/${accountId}/detail`)
        .then(detail => {
          const merged = {
            ...base,
          ...
          merged.avatarUrl = gravatarUrl(merged.email || `${accountId}@example.invalid`);
          this.accountCache.set(accountId, merged);
        })
        .catch(() => {
          // keep base profile
        });
    ...
    showAccountHovercard(anchorEl, msg) {
      const accountId = getAuthorAccountId(msg);
      if (!accountId || !this.accountHovercardEl) return;
      this.cancelHideAccountHovercard();

      const profile = this.accountCache.get(accountId) || {
        id: accountId,
        name: getAuthorName(msg) || `a/${accountId}`,
        email: getAuthorEmail(msg, accountId, getAuthorName(msg)),
        username: null,
        avatarUrl: getAuthorAvatarUrl(msg, accountId, getAuthorName(msg)),
      };

      this.accountHovercardEl.innerHTML = `
        <div class="account-hovercard-head">
          <img class="account-hovercard-avatar" src="${escapeHtmlAttr(profile.avatarUrl)}" alt="avatar" referrerpolicy="no-referrer" />
          <div>
            <div class="account-hovercard-name">${escapeHtml(profile.name || `a/${accountId}`)}</div>
            <div class="account-hovercard-meta">a/${accountId}${profile.username ? ` • @${escapeHtml(profile.username)}` : ''}</div>
          </div>
        </div>
        ${profile.email ? `<div class="account-hovercard-row">${escapeHtml(profile.email)}</div>` : ''}
      `;

      const rect = anchorEl.getBoundingClientRect();
      const card = this.accountHovercardEl;
      card.classList.add('open');
      card.style.left = `${Math.max(8, Math.round(rect.left))}px`;
      card.style.top = `${Math.round(rect.bottom + 8)}px`;
    }

    hideAccountHovercard() {
      this.cancelHideAccountHovercard();
      if (!this.accountHovercardEl) return;
      this.accountHovercardEl.classList.remove('open');
    }

    scheduleHideAccountHovercard() {
      this.cancelHideAccountHovercard();
      this.accountHovercardHideTimer = window.setTimeout(() => {
        this.hideAccountHovercard();
      }, 120);
    }

    cancelHideAccountHovercard() {
      if (this.accountHovercardHideTimer) {
        window.clearTimeout(this.accountHovercardHideTimer);
      }
      this.accountHovercardHideTimer = null;
    }
    ...
    renderPendingMessage() {
      ...
      label.className = 'pending-step-text';
      const step = this.progressSteps[this.progressStepIndex] || 'working';
  ...
  function getAuthorEmail(msg, accountId, authorName) {
    const email = msg && (msg.authorEmail ?? msg.author_email);
    if (email && String(email).trim()) {
      return String(email).trim().toLowerCase();
    }
    return fallbackSeedEmail(accountId, authorName);
  }

  function getAuthorAvatarUrl(msg, accountId, authorName) {
    const direct = msg && (msg.authorAvatarUrl ?? msg.author_avatar_url);
    if (direct && String(direct).trim()) {
      return String(direct).trim();
    }

    const email = getAuthorEmail(msg, accountId, authorName);
    return gravatarUrl(email);
  }

  function fallbackSeedEmail(accountId, authorName) {
    const safeName = String(authorName || accountId || 'user')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '.')
      .replace(/^\.+|\.+$/g, '');

    if (accountId) {
      return `account.${accountId}@example.invalid`;
    }

    return `${safeName || 'user'}@example.invalid`;
  }

  function gravatarUrl(email) {
    return `https://www.gravatar.com/avatar/${md5Hex(String(email || '').trim().toLowerCase())}?d=identicon&s=64&r=g`;
  }
  ...
  function md5Hex(input) {
    ...
    function toWords(str) {
      ...
      const words = [];
      for (let i = 0; i < len; i++) {
        words[i >> 2] = (words[i >> 2] || 0) | (msg.charCodeAt(i) << ((i % 4) * 8));
      }
    ...
    for (let i = 0; i < M.length; i += 16) {
      ...
      for (let j = 0; j < 64; j++) {
        ...
        D = C;
        C = B;
        B = (B + leftRotate((A + F + K[j] + (M[i + g] || 0)) >>> 0, s[(j >> 4) * 4 + (j % 4)])) >>> 0;
        A = tmp;
      }

Gerrit.install(plugin => {
  ...
  class GerritChatWidget extends HTMLElement {
    ...
    startProgress() {
      ...
      this.statusEl.textContent = 'Working…';

      this.progressTimer = window.setInterval(() => {
        this.progressStepIndex = (this.progressStepIndex + 1) % this.progressSteps.length;
        this.statusEl.textContent = `Working… ${this.progressSteps[this.progressStepIndex]}`;
        this.renderMessages();
      }, 1000);
    }
    ...
    renderPendingMessage() {
      ...
      label.textContent = `Working… ${step}`;
      ...
      toolsHint.textContent = 'Tool calls appear as collapsed entries once available.';
